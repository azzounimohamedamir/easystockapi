...
namespace SmartRestaurant.Application.IntegrationTests.Orders.Commands
{
    ...
    [TestFixture]
    public class CreateOrderSHCommandRestaurantWithQuantity : TestBase
    {
        [Test]
        public async Task CreateOrderSHCommandRestaurantWithParamShouldSaveToBdd()
        {
            ...
            var createParkingService = await HotelServicesTestTools.CreateHotelParkingServiceWithQuantityBySectionId(createSheratonMallSection.Id, fastFood.FoodBusinessId.ToString(),300,true);
            var checkin = await CheckInsTestTools.CreateCheckinForClient(hotel.Id, client.Id, room.Id);
            

         

            var OrderSHForHotelServiceWithParameteres = await OrderTestTools.CreateOrderSHwithQuantity(fastFood, createParkingService, checkin, hotel,5);
            ...
            OrderSHForHotelServiceWithParameteres.TotalToPay.Should().Be(1500);
            OrderSHForHotelServiceWithParameteres.UnitePrice.Should().Be(300);
