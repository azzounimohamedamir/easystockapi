...
namespace SmartRestaurant.Application.IntegrationTests.Bills.Queries
{
    ...
    [TestFixture]
    public class GetBillsLisTest : TestBase
    {   
        [Test]
        public async Task GetOrdersList()
        {
            ...
            var dishesList_02 = await SendAsync(new GetBillsListQuery { CurrentFilter = "number", SearchKey = "0001" , FoodBusinessId = fastFood.FoodBusinessId.ToString() });
            dishesList_02.Data.Should().HaveCount(1);

            var dishesList_03 = await SendAsync(new GetBillsListQuery { CurrentFilter = "number", SearchKey = "0002", FoodBusinessId = fastFood.FoodBusinessId.ToString() });
            dishesList_03.Data.Should().HaveCount(0);

            var dishesList_04 = await SendAsync(new GetBillsListQuery { CurrentFilter = "number", DateInterval = DateFilter.ToDay, FoodBusinessId = fastFood.FoodBusinessId.ToString() });
            dishesList_04.Data.Should().HaveCount(1);

            var dishesList_05 = await SendAsync(new GetBillsListQuery { CurrentFilter = "number", SearchKey = "test", FoodBusinessId = fastFood.FoodBusinessId.ToString() });
