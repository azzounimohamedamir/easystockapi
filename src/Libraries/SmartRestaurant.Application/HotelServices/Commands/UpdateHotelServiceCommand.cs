...
{
	 public class UpdateHotelServiceCommand:UpdateCommand
	 {
	 ...
	 public class UpdateHotelServiceCommandValidator : AbstractValidator<UpdateHotelServiceCommand>
	 {
		  public UpdateHotelServiceCommandValidator()
		  {
					 RuleFor(m => m.SectionId).NotEmpty().NotEqual(Guid.Empty).WithMessage("'{PropertyName}' must be a valid GUID");
				RuleFor(m => m.Id).NotEmpty().NotEqual(Guid.Empty).WithMessage("'{PropertyName}' must be a valid GUID");
