...
namespace SmartRestaurant.Application.Orders.Commands
{
			...
			IRequestHandler<UpdateOrderGeoLocalisationCommand, Order>
	{
		...
		)
		{
			...
			var foodBusiness = await _context.FoodBusinesses.FindAsync(
				Guid.Parse(request.FoodBusinessId)
			);
			...
			{
				var isOutdeliveryTime =
					DateTimeHelpers.CheckAvailabiliteOfOrderDeliveryInCurrentTime(
						foodBusiness.OpeningTime,
						foodBusiness.ClosingTime,
						_datetime
					);
				...
				{
					var newOrder = await ExecuteOrderOperations(
						request,
						cancellationToken,
						foodBusiness
					);
					newOrder.ErrorDeliveryTimeAvailabilite = ErrorResult.None;
					await addOrderNotifications(
						newOrder.OrderId.ToString(),
						newOrder.FoodBusinessId.ToString(),
		...
		private Order PopulatFromLocalDishesAndProducts(Order order)
		{
			order.Dishes = order.Dishes
				.Select(
					(OrderDish od) =>
					{
						var orderDish = _context.Dishes.FirstOrDefault(
							d => d.DishId.Equals(Guid.Parse(od.DishId))
						);
						if (orderDish == null)
						...
						od.Supplements = od.Supplements
							.Select(
								(supplement) =>
								{
									var supp = _context.Dishes.FirstOrDefault(
										d => d.DishId.Equals(Guid.Parse(supplement.SupplementId))
									);
			...
			order.Products = order.Products
				.Select(
					(OrderProduct p) =>
					{
						var orderProduct = _context.Products.FirstOrDefault(
							d => d.ProductId.Equals(Guid.Parse(p.ProductId))
						);
		...
		public async Task<OrderDto> ExecuteOrderOperations(
			CreateOrderCommand request,
			CancellationToken cancellationToken,
		...
		)
		{
			if (request.FoodBusinessClientId != null)
			{
				var foodBusinessClient = await _context.FoodBusinessClients.FindAsync(
					Guid.Parse(request.FoodBusinessClientId)
				);
				if (foodBusinessClient == null)
					throw new NotFoundException(
						nameof(FoodBusinessClient),
						request.FoodBusinessClientId
					);
			}
			...
			order.CreatedBy = ChecksHelper.GetUserIdFromToken_ThrowExceptionIfUserIdIsNullOrEmpty(
				_userService
			);
			...
			orderDto.CurrencyExchange = CurrencyConverter.GetDefaultCurrencyExchangeList(
				orderDto.TotalToPay,
				foodBusiness.DefaultCurrency
			);
		...
		)
		{
			...
			await addOrderNotifications(
				...
				order.FoodBusinessId.ToString(),
				OrderNotificationType.Update,
				cancellationToken
			);
			return default;
		}
		...
		)
		{
			...
			if (order.Status == OrderStatuses.Billed)
				throw new ConflictException(
					"Sorry, you can not change the status for a Billed Order"
				);
		...
		public async Task<Order> Handle(
			UpdateOrderGeoLocalisationCommand request,
			CancellationToken cancellationToken
		)
		{
		...
		public async Task<NoContent> Handle(
			AddSeatOrderToTableOrderCommand request,
			CancellationToken cancellationToken
		)
		{
			...
			if (!result.IsValid)
				throw new ValidationException(result);
			Order order = await GetOrderForUpdate(request.Id, cancellationToken)
				.ConfigureAwait(false);
		...
		{
			order.Dishes.RemoveAll(
				x => x.TableId == request.TableId && x.ChairNumber == request.ChairNumber
			);
		...
		private void CalculateAndSetOrderNumber(
			Order order,
			Domain.Entities.FoodBusiness foodBusiness
		)
		{
			...
			var orderNumberLastResetDateTimeInUtc = TimeZoneInfo.ConvertTimeToUtc(
				foodBusiness.OrderNumberLastResetDateTime
			);
			var orderNumberLastResetDateTimeInAlgeriaTime = TimeZoneInfo.ConvertTimeFromUtc(
				orderNumberLastResetDateTimeInUtc,
				algeriaZone
			);
		...
		private void CalculateAndSetOrderTotalPrice(
			Order order,
			Domain.Entities.FoodBusiness foodBusiness
		)
		{
			order.CommissionConfigs =
				foodBusiness.CommissionConfigs == null
					? new CommissionConfigs()
					: foodBusiness.CommissionConfigs;
			if (
				foodBusiness.CommissionConfigs == null
				|| foodBusiness.CommissionConfigs.WhoPay == WhoPayCommission.FoodBusiness
			)
			{
		...
		private void AddCommissionValueToTotalPrice(
			Order order,
			Domain.Entities.FoodBusiness foodBusiness
		)
		{
			if (
				order.Type == OrderTypes.DineIn
				&& foodBusiness.CommissionConfigs.Type == CommissionType.PerPerson
			)
			{
				var personsCount = 0;
			...
			foreach (var dish in order.Dishes)
			{
				...
				foreach (var ingredient in dish.Ingredients)
				{
					...
					if (ingredientDtails.EnergeticValue.Amount == 0)
					{
						...
						OrderishIngredient.MeasurementUnits
						== ingredientDtails.EnergeticValue.MeasurementUnit
					)
					{
						energeticValues.Add(
							(
								(OrderishIngredient.Amount * ingredientDtails.EnergeticValue.Energy)
								/ ingredientDtails.EnergeticValue.Amount
							)
						);
					}
					else
						...
						switch (OrderishIngredient.MeasurementUnits)
						{
							case MeasurementUnits.Gramme:
								energeticValues.Add(
									(
										(
											OrderishIngredient.Amount
											* (ingredientDtails.EnergeticValue.Energy / 1000)
										) / ingredientDtails.EnergeticValue.Amount
									)
								);
		...
		private void ChangeStatusForOccupiedTablesOnlyIfOrderTypeIsDineIn(
			Order order,
			string action
		)
		{
		...
		private void SetTabelOccuped(string action, OrderOccupiedTable occupiedTable)
		{
			var table = _context.Tables
				.AsNoTracking()
				.FirstOrDefault(t => t.TableId == Guid.Parse(occupiedTable.TableId));
			if (table == null)
				throw new NotFoundException(nameof(Tables), occupiedTable.TableId);

			if (table.TableState == TableState.Archived)
				throw new ConflictException(
					$"The table numbered with '{table.TableNumber.ToString().PadLeft(3, '0')}' can not be used because it is archived"
				);
		...
		private void ChangeStatusForReleasedTablesOnlyIfOrderTypeIsDineIn(
			List<string> releasedTables
		)
		{
			foreach (var tableId in releasedTables)
			{
				var table = _context.Tables
					.AsNoTracking()
					.FirstOrDefault(t => t.TableId == Guid.Parse(tableId));
				if (table == null)
		...
		private List<string> GetReleasedTables(Order order, UpdateOrderCommand request)
		{
			...
			{
				var result = request.OccupiedTables.Find(
					x => x.TableId == orderOccupiedTable.TableId
				);
				if (result == null)
		...
		public TimeZoneInfo GetAlgeriaTimeZone()
		{
			...
			if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
			{
				throw new NotImplementedException(
					"I don't know how to do a lookup for time zone on a Mac."
				);
			}
			return default;
		}
		...
		private async Task UpdateDishesAndProductQuantityOnCreateOrder(Order order)
		{
			...
			foreach (var dish in dishes)
			{
				...
				if (dishUpdated.IsQuantityChecked && dishUpdated.Quantity > 0)
				{
					...
					{
						throw new ConflictException(
							"Sorry, you can not take this quantity : "
								+ dish.Quantity
								+ " , issue quantity. "
						);
					}
			...
			{
				var productUpdated = await _context.Products.FindAsync(
		...
		private async Task addOrderNotifications(
			string orderId,
			string foodBusinessId,
			OrderNotificationType type,
			CancellationToken cancellationToken
		)
		{
			var foodBusiness = await _context.FoodBusinesses
				.Where(u => u.FoodBusinessId == Guid.Parse(foodBusinessId))
				.FirstOrDefaultAsync()
				.ConfigureAwait(false);

			if (foodBusiness == null)
				throw new NotFoundException(nameof(foodBusiness), foodBusinessId);
			...
			await _fireBase.AddCollectionAsync(
				pathNotification,
