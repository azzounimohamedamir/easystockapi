    ...
    public class FilterProductsByDescription : IProductFilterStrategy
    {
        ...
        private static Expression<Func<Product, bool>> Condition(string foodBusinessId, string searchKey)
        {
            if (foodBusinessId == null)
                return product => product.Description.Contains(searchKey) && product.FoodBusinessId == null;
            else
                return product => product.Description.Contains(searchKey) && product.FoodBusinessId == Guid.Parse(foodBusinessId);
        }
