...
namespace SmartRestaurant.API.Controllers
{
    [Route("api/vbc")]
    [ApiController]
    [SwaggerTag("List of actions that can be applied on Bon commande client")]
    public class BonCommandeController : ApiController
    {
        ...
        {
            return await SendWithErrorsHandlingAsync(new DeleteBCCommand { Id = id });
        }

        /// <summary> TransformerBcenBL </summary>
        /// <remarks>
        /// </remarks> 
        ...
        [ProducesResponseType(typeof(ExceptionResponse), 400)]
        [HttpPost]
        [Route("transformerBCEnBL")]

        // [Authorize(Roles = "FoodBusinessManager,FoodBusinessAdministrator,SuperAdmin,SupportAgent")]
        public async Task<IActionResult> TransformerBCenBL(TransformerBCenBLCommand command)
        {
            return await SendWithErrorsHandlingAsync(command);
        }
    }
