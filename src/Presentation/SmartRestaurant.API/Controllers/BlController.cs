...
namespace SmartRestaurant.API.Controllers
{
    ...
    public class BlController : ApiController
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllBLs(string currentFilter, int page, int pageSize, int caisse)
        {
           return await SendWithErrorsHandlingAsync(new GetBonLivraisonsListQuery
            {
       ...
       // [Authorize(Roles = "FoodBusinessManager,FoodBusinessAdministrator,SuperAdmin,SupportAgent")]
        public async Task<IActionResult> Create(CreateBlCommand command)
        {
        ...
        [HttpPatch]
        [Route("groupBls")]
        public async Task<IActionResult> GroupBls(RegroupBlCommand command)
        {
            return await SendWithErrorsHandlingAsync(command);
        }

        /// <summary> TransformerBLenFacture </summary>
        /// <remarks>
        /// </remarks> 
        ...
        [ProducesResponseType(typeof(ExceptionResponse), 400)]
        [HttpPost]
        [Route("transformerEnFacture")]

        // [Authorize(Roles = "FoodBusinessManager,FoodBusinessAdministrator,SuperAdmin,SupportAgent")]
        public async Task<IActionResult> TransformerEnFacture(TransformerBLenFactureCommand command)
        {
            return await SendWithErrorsHandlingAsync(command);
        }
        ...
        [Route("{id:guid}")]
        [HttpGet]
        public async Task<IActionResult> GetFacByid([FromRoute] Guid id)
        {
